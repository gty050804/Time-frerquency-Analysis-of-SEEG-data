% いま，Trigger开始时与baseline有一定程度的区分，但是按道理来说Trigger后3s为动作执行阶段，应该具有更强的
% 相关性，但是在实验结果中没有看到这种强相关性，Trigger后3s的相关性比Trigger刚开始时的相关性更小。
% （Trigger的实际意义理解错误？）
...
% sel_Chn = 20;
extent = 4;     % 决定Trigger附近多短的时间（单位：s）
...
for subjId = targetSubjects
    ...
    for cons = sessionNo
        ...
        g4 = extent * actualFs;               % 特定时间内的采样点数，4=2+2
        ...
        for chn = 1:g2
            ...
            for trial = 1:g3  % the segment number (i th)
                    ...
                    count = count + 1;
                        ...
                        A1 = selected_Chn( ...
                            max(1,index_of_stimulus_onset(trial)-actualFs*extent/2): ...
                            min(index_of_stimulus_onset(trial)-1,length(Trig)));

                        A2 = selected_Chn( ...
                            max(1,index_of_stimulus_onset(trial) + 3*actualFs): ...
                            min(index_of_stimulus_onset(trial)+actualFs*extent/2-1+3*actualFs,length(Trig)));
